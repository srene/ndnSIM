...
{
	NFD_LOG_FACE_TRACE(this<<""<<m_forwarder);
...
InrppLinkService::PullPacketFromCS()
{
	...
	std::shared_ptr<InrppForwarder> f = std::dynamic_pointer_cast<InrppForwarder>(m_forwarder);
	//auto f = dynamic_cast<InrppForwarder*>(m_forwarder.get());
	NFD_LOG_DEBUG("Forwarder "<< f <<" "<<m_forwarder.get()<<" "<< getFace()->getBps() << " "<< getFace()->getId());
	f->sendData(getFace()->getId());
