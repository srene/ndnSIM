...
InrppStackHelper::Install(Ptr<Node> node) const
{
  ...
  ndn->setForwarder(fw);
  NS_LOG_LOGIC("Inrppforwarder "<< fw << " "<< ndn->getForwarder()<<" "<<std::dynamic_pointer_cast<nfd::InrppForwarder>(ndn->getForwarder()));
...
std::string
InrppStackHelper::constructFaceUri(Ptr<NetDevice> netDevice) const
{
                                           ...
                                           Ptr<NetDevice> device) const
{
  ...
  NS_LOG_LOGIC("Datarate "<<netDevice->GetDataRate().GetBitRate()<<" "<< std::dynamic_pointer_cast<nfd::InrppForwarder>(ndn->getForwarder())<<" "<<ndn->getForwarder());
  ...
  auto face = std::make_shared<Face>(std::move(linkService), std::move(transport));
  face->setMetric(1);
  face->setBps(netDevice->GetDataRate().GetBitRate());
  ndn->addFace(face);
  NS_LOG_LOGIC("Node " << node->GetId() << ": added Face as face #"
                       << face->getLocalUri());
